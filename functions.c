...
void displayStudents(Student students[], int *count) { 
    system("cls");
	if (*count == 0) {//Kiem tra danh sach trong
    ...
    printf("|==========|====================|=============================|====================|=============|\n");
    int i;
    for (i = 0; i < *count; i++) {//In ra danh sach sinh vien
...
void addStudents(Student students[], int *count) {
    ...
    printf("Enter Number of Courses: ");//Nhap so Course
...
void editStudents(Student students[], int *currentLength){
	system("cls");//Xoa va kiem tra danh sach trong
	if (*currentLength == 0) {
...
void deleteStudents(Student students[], int *currentLength){
    system("cls");//Xoa man hinh va kiem tra danh sach trong
	if (*currentLength == 0) {
...
// Dien thong tin admin
void registerAdmin(const char *filename) {
    char emailAdmin[50], passwordAdmin[50];
    ...
    fgets(passwordAdmin, sizeof(passwordAdmin), stdin);
    passwordAdmin[strcspn(passwordAdmin, "\n")] = '\0';
    fprintf(file, "%s\n%s\n", emailAdmin, passwordAdmin); // Ghi email va password vao file
    fclose(file);
...
void loginAdmin(const char email[], const char password[]) {
    ...
    FILE *file = fopen("addminFile.data", "r");
    if (file == NULL) {
        printf("Error: Admin credentials not found. Please register first.\n");
        return;
    }
    fgets(savedEmail, sizeof(savedEmail), file);
    savedEmail[strcspn(savedEmail, "\n")] = '\0';
    ...
    while (attempt < maxAttempt){
        ...
        if (strcmp(inputEmail, savedEmail) == 0 && strcmp(inputPassword, savedPassword) == 0) {
            ...
            break;
        } else if (strcmp(inputEmail, savedEmail) != 0) {
